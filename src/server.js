...
//
// By default the server acts as the signed-in admin (delegated auth), so every
// action respects that admin's Purview RBAC; both planes also support
// certificate-based app-only auth for headless hosts (see README). Structure
// mirrors nvd-mcp-local: the low-level Server API, compact/markdown
// formatters, and prompts that chain tools into review workflows.
//
// createServer() builds a fresh Server instance per call so the same tool
...
// MCP tool annotations, per behaviour class. Reads never mutate; creates are
// additive (re-running one errors on the duplicate rather than overwriting);
// sets overwrite existing tenant state; removes destroy it. Every tool talks
// to a remote tenant, hence openWorldHint throughout.
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "Get full details of a single sensitivity label (color, tooltip, parent, sensitivity order) by its label ID. Set include_protection_settings to also return the protection the label applies (encryption, content marking, container and Teams protection) — the settings that create_sensitivity_label/set_sensitivity_label write.",
    ...
    inputSchema: {
      ...
      properties: {
        ...
        include_protection_settings: {
          type: "boolean",
          description: "Also return the label's protection configuration (encryption, marking, container/Teams). Needs the PowerShell plane.",
        },
      },
    },
  },
  ...
  },
  {
    name: "set_dlp_policy",
    description:
      "Modify an existing DLP policy (Set-DlpCompliancePolicy). Primary uses: change a policy's mode to move it between a Test mode and enforcement (Enable), or add/remove the locations it protects. Only supplied fields change. WRITE operation — this changes tenant configuration.",
  ...
  },
  {
    name: "set_dlp_rule",
    description:
      "Modify an existing DLP rule (Set-DlpComplianceRule) — traditional, endpoint, or Copilot. Only supplied fields change. WRITE operation.",
    ...
    inputSchema: {
      ...
      properties: {
        ...
        sensitive_information_types: {
          type: "array",
          items: { type: "string" },
          description: "Replace the detected sensitive information types, e.g. ['Credit Card Number'].",
        },
...
async function dispatch(name, args) {
  switch (name) {
    ...
    case "get_sensitivity_label": {
      ...
      if (!args.include_protection_settings) return text(detail);
      // The protection settings live on the PowerShell plane; degrade to the
      // Graph half with a clear note rather than failing the whole call.
      let protection;
      try {
        protection = labels.formatLabelProtectionSettings(await labels.getLabelProtectionSettings(args.label_id));
      } catch (err) {
        protection = `**Protection settings unavailable:** ${err.message}`;
      }
      return text(`${detail}\n\n${protection}`);
    }
...
/**
 * Build a fresh, fully-wired MCP Server. The stdio entry point (index.js)
